  ...
  return (
    <div style={{ position: 'relative' }}>
      ...
      <svg ref={ref} width={W} height={H}
        ...
        <text x={W - PAD.right + 3} y={tY + 5} fontSize="14" fill="#94a3b8" opacity="1" fontWeight="700">150K</text>
  ...
  return (
    <div style={{
      ...
      <div style={{ flex: '0 0 170px', display: 'flex', flexDirection: 'column', gap: 10 }}>
        <div style={T.label}>Ads Performance</div>

        <div>
          <div style={{ ...T.tiny, marginBottom: 3 }}>CPRL</div>
        ...
        {/* Channels */}
        <div style={{ borderTop: '1px solid rgba(255,255,255,0.09)', paddingTop: 8, display: 'flex', flexDirection: 'column', gap: 6 }}>
          {CHANNELS.map(ch => {
            const v = vals[ch.key], pct = total > 0 ? (v / total) * 100 : 0
            return (
              <div key={ch.key}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: 3 }}>
                  <span style={{ fontSize: 9, fontWeight: 700, color: ch.color, letterSpacing: '0.07em' }}>{ch.label}</span>
