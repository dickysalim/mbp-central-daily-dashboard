/**
 * LeadsQualityCard — v3
 * Left: CPA CC headline + status + Purchase CC
 * Right: CPA CC trend chart — badge below chart
 */
...
import { SKU_COLORS, skuColor } from '../../utils/skuColors'
import { T } from '../../utils/tokens'

const CPA_CC_TARGET = 2_000_000

export interface CpaPoint    { date: string; value: number }
...
function CpaChart({ data, changelog }: { data: CpaPoint[]; changelog: ChangelogRow[] }) {
  ...
  const [tooltip, setTooltip] = useState<{ x: number; y: number; p: CpaPoint } | null>(null)
      ...
      <svg ref={ref} width={W} height={H}
        ...
        {tooltip && (
          <g>
            ...
            <circle cx={tooltip.x} cy={tooltip.y} r="4.5" fill="#f472b6" stroke="#0d0e12" strokeWidth="1.5" />
          </g>
        )}
